 ...
 *
 * To compile:
 *   mpicc -o MPI-linear-regression MPI-linear-regression.c -lm
 *
 * To run:
 *   mpirun -n 9 ./MPI-linear-regression
 *
...
double residual_error (double k, double s, double m, double c)
{
   double e = (m * k) + c - s;
   return e * e;
}
...
double rms_error (double m, double c)
{
   ...
   for (i = 0; i < n_data; i++)
   {
      error_sum += residual_error (data[i].k, data[i].s, m, c);
   }
